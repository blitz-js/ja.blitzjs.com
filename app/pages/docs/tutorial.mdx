...
ここでは、すでに Blitz がインストールされている( [Blitz のインストール方法](./get-started]) を参照ください)ことを前提とします。Blitz がインストールされているかどうかは、ターミナルで以下のコマンドを実行することでわかります。
...
ターミナルで`cd` コマンドを用い、アプリを作成したいフォルダに移動し、次のコマンドを実行します。

```sh
blitz new my-blitz-app
```

Blitz は現在のフォルダ内に `my-blitz-app` フォルダを作成します。フォームライブラリを選択するように促されます。このチュートリアルでは、推奨されるライブラリ `React Final Form` を選択します。
...
- `app/pages/` ディレクトリは、主要なページフォルダです。Next.js を使ったことがある人なら、この違いにすぐに気づくでしょう。Blitz では多くの`pages`フォルダを持つことができ、それらはビルド時にマージされます。
...
それでは、`my-blitz-app` フォルダを確認しましょう。まだ `my-blitz-app` フォルダが存在しない場合は、次のコマンドを実行してください。
...
Blitz アプリは、ユーザーの Sign Up(会員登録)やログインがすでに設定されている状態で作成されています。
早速試してみましょう。**Sign Up** ボタンをクリックします。任意のメールアドレスとパスワードを入力して、**Create Account**をクリックします。そうすると、あなたのユーザ `id` と `role` が表示されるホームページにリダイレクトされます。

必要であれば、ログアウトしてからログインし直すこともできます。あるいは、ログインページの**Forgot your password?** をクリックして、その流れを試してみてください。

## 最初のページの実装 {#write-your-first-page}

次に、最初のページを作成してみましょう。

ファイル `app/pages/index.tsx` を開き、`Home` コンポーネントの内容をこれに置き換えます。
...
保存するとブラウザでページが更新されるはずです。これは好きなだけカスタマイズを追加できます。準備ができたら、次のセクションに進みましょう。

## データベースの設定 {#database-setup}

嬉しいことに、SQLite を用いたデータベースの設定がすでに作成されています。ターミナルで`blitz prisma studio` を実行すると、データベース内のデータを見ることができる Web インターフェースが開きます。

最初の本格的なプロジェクトを始めるときには、PostgreSQL のようなスケーラブルなデータベースを使用した方が良いでしょう。詳細については、[データベースの概要](database-overview) を参照してください。今のところ、デフォルトの SQLite データベースを使用します。

## モデルのコードの自動生成 {#scaffolding-code-for-our-models}

Blitz には [`generate`](./cli-generate) という便利な CLI コマンドがあります。ここでは、`generate`を使って `Question`（質問）と `Choice`（選択肢）の 2 つのモデルを作成します。 `Question` は質問のテキストと選択肢のリストを持ちます。選択肢には、選択肢のテキスト、投票数、関連する質問が含まれます。Blitz は両方のモデルについて、ID、作成タイムスタンプ、最終更新タイムスタンプを自動的に生成します。
...
ターミナルが表示されたら、**Enter** を押して `prisma migrate` （マイグレーション） を実行してください。名前を聞かれるので、"add question" と入力してください。
...
これはデータベースの `Question` モデルに実際のフィールドが追加されていないため、データベースのマイグレーションを必要としません。
...
Prisma はまだ "カスケード削除 "をサポートしていません。このチュートリアルでは、`Question` を削除する際に `Choice` データを削除しないことを意味します。
これを手動で行うためには、生成された`deleteQuestion` Mutation を一時的に拡張する必要があります。テキストエディタで`app/questions/mutations/deleteQuestion.ts` を開き、関数本体の先頭に以下を追加してください。
...
いいですね！localhost が実行されていることを確認してください。実行されていない場合、ターミナルで`blitz dev` を実行し、`localhost:3000/questions` にアクセスしてください。問題の作成、編集、削除を試してみてください。
...
それから `app/questions/mutations/createQuestion.ts` を開き、Mutation で選択肢のデータが受け入れられるように zod スキーマを更新します。また、選択肢が作成されるように `db.question.create` コールを更新する必要があります。
...
これで、`localhost:3000/questions/new` を開いて、新しい問題と選択肢を作成することができます。

## リストの選択 {#listing-choices}

休憩です。ブラウザの `localhost:3000/questions` に戻って、作成したすべての問題を見てください。ここに質問の選択肢をリストアップしてみましょう。 まず、質問のクエリをカスタマイズする必要があります。
Prisma では、ネストされた関係をクエリしたいことをクライアントに手動で知らせる必要があります。あなたの `getQuestion.ts` と `getQuestions.ts` ファイルを以下のように変更してください。
...
エディタでメインの問題ページ (`app/pages/questions/index.tsx`) に戻って、各問題の選択肢をリストアップしてみましょう。そして、このコードを `QuestionsList` の `Link` の下に追加してください。
...
1. `<h1>Question {question.id}</h1>` を `<h1>{question.text}</h1>` に置き換える。
...
次に、更新した `updateChoice` Mutation をインポートして `handleVote` 関数を作成する。
...
そして、`handleVote`の中で使用する関数 `refetch` を返すために質問 `useQuery` コールを更新する必要があります。
...
既存の問題の **Edit** ボタンをクリックすると、問題を作成するのと同じフォームを使用していることがわかります。これですでに完了です。あとは Mutation を更新するだけです。
...
[`upsert`](https://www.prisma.io/docs/reference/api-reference/prisma-client-reference#upsert) は、「この項目が存在すればそれを更新する、存在しなければ作成する」という意味です。質問を作成する際にユーザーが 3 つの選択肢を追加する必要がないので、このケースには最適です。後でユーザーが質問を編集して別の選択肢を追加した場合、ここに作成されます。
...
あなたのプロジェクトを世界中の Blitz コミュニティと共有したい場合、Discord 以上に良い場所はありません。

[discord.blitzjs.com](https://discord.blitzjs.com)にアクセスしてください。そして、**#built-with-with-blitz** チャンネルにリンクを投稿して、みんなで共有しましょう！
